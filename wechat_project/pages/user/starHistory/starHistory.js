// pages/user/starHistory/starHistory.js
import { getVoice,getStarNews } from "../../../utils/request"
...
Page({
  ...
  onLoad(options) {
    ...
    getStarNews(this.data.pageNumber, this.data.pageSize).then(res=>{
      this.setData({
        articles: res.data,
        ...
        if (newVal == CONTROLSTATUS.up) {
          if (this.data.activeId > 0) {
            ...
            if (!this.data.refreshView) {
              ...
              wx.reLaunch({
                url: '/pages/user/starHistory/starHistory.js',
              })
            }
          }
  ...
  onShow() {
    initAudioController()
    readController("voice/star/starHistory.mp3", "control")
  },
