...
def recmd():
    ...
    print(user["user_id"] + " " + user["how_many"])
    print("新闻推荐启动")
    data = {
        "news": recommender.myrecommender(int(user["user_id"]), int(user["how_many"]))
    }
    response.content_type = 'application/json'
    return json.dumps(data)
...
app.run(host='10.11.12.22', port=18080, reloader=True)
