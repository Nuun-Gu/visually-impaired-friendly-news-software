...
def wavtobase64(base64_str):
    myuuid = uuid.uuid1()
    cache_path = str(myuuid) + "cache.wav"
    wav_out = cache_path
...
def getVoice(text):
    ...
    sender_response = myRequest('POST', servlet_path=send_query_servlet_path,
                                data=voice_sender(text), headers=sender_fetch_headers())
    base64_str = sender_response['body']['data']
    #print(sender_response)
    return "data:audio/wav;base64," + str(wavtobase64(base64_str), 'ascii', 'ignore')
